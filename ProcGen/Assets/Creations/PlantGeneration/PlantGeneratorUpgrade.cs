...
public class PlantGeneratorUpgrade : MonoBehaviour
{
    [SerializeField]
    ...
    void FixedUpdate()
    {
        if (timer >= timeLimit)
        {
            ...
            {
                Debug.Log("process rules");
                ProcessStructureRules();
            }
            else//(isUpdated == false)
            {
                Debug.Log("update graphics");
                UpdateGraphics();
            }
            timer = 0;
        }
    ...
    private void DestroyPreviousNLevel()
    {
        int length = gameObjectPositions.Count;
        for (int i = 0; i < length; i++)
        {
            Destroy(gameObjectPositions[i].gameObject, 0.1f);
        }
    ...
    public class RuleA
    {
        ...
        public static void DrawingRule(int index, int nLevel, out Vector3 outPosition, out Quaternion outRotation)
        {
            Vector3 position;
            Quaternion rotation;
            position = new Vector3(index, 0.5f, 0);
